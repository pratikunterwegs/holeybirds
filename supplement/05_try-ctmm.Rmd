...
```{r}
# load data from file
data <- fread("data/processed/data_preprocessed/data_preproc_Acrocephalus_Manipulated_1004001585.csv")
```
...
df[, posixtime := as.POSIXct(UNIX, tz = "Asia/Jerusalem", origin = "1970-01-01")]
df[, c("date", "hour") := list(
  date(posixtime),
  hour(posixtime)
)]
...
# df_day$posixtime <- as.POSIXct(df_day$time, origin = "1970-01-01")
