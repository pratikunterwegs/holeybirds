...
data_raw = data_raw[vapply(data_raw, function(df) { nrow(df) > 500 }, FUN.VALUE = T)]
...
data_proc = data_proc[vapply(data_proc, function(df) { nrow(df) > 500 }, FUN.VALUE = T)]
...
# now smooth
data_proc = lapply(data_proc, atl_median_smooth, x = "x", y = "y",
...
data_agg = lapply(data_proc, function(df) {
  ...
  df = atl_thin_data(data = df, interval = 120, 
