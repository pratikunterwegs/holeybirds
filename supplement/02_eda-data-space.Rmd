...
```{r}
data_raw <- fread("data/processed/data_preprocessed/data_preproc_Passer_1004001335.csv")
unique(data_raw$TAG)
nrow(data_raw)

# split by day
data_raw <- split(data_raw, by = "date")
...
data_proc <- lapply(data_proc, function(df) {
  atl_filter_covariates(
    df,
    filters = c(
      "((!outlier)) | ((angle < 10) & (angle != 0) & (speed_in > 3) & (speed_out > 3))"
    )
  )
})
