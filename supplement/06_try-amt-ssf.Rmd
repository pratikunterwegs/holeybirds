...
steps$geometry = pmap(steps, function(x1_, y1_, x2_, y2_) {
  # make linear path
  step_path = st_as_sfc(
    sprintf("LINESTRING(%f %f, %f %f)", x1_, y1_, x2_, y2_)
  )
  
  # make sample points
  step_sample_points = st_line_sample(step_path, density = 1, type = "regular")
  
  # cast to simple point
  step_sample_points = step_sample_points %>% 
    st_cast("POINT")
  
  # count points and remove first and last
  if (length(step_sample_points) > 3) {
    step_sample_points = step_sample_points[2:(length(step_sample_points) - 1)]
    step_sample_points = st_union(step_sample_points)
    step_geom = st_cast(step_sample_points, "LINESTRING")
...
# make sf all around
ndvi_path = vapply(steps$geometry, function(x) {
  ndvi_velox$extract(sp = x, fun = mean)
}, FUN.VALUE = 1.0)
```
