...
```{r}
# list files
files = list.files("data/processed/data_preprocessed", pattern = "thin", 
    full.names = TRUE)
```
...
### Filter IDs with fewer than N daily fixes

```{r filter_id_day}
# minimum daily fixes -- multiply by 20 for tracking duration in seconds
min_daily_fixes = 100

# filter out days < 500 fixes
data_day_count = data_day_count[N > min_daily_fixes, ]

# save data daily count
fwrite(data_day_count, "data/results/data_daily_count_good.csv")

# join to data and remove data with too few fixes
...
# write data
fwrite(data, file = "data/processed/data_thinned_for_analysis.csv")
```
