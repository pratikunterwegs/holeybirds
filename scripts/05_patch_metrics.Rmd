...
# asign sp as factor
patch_switches[, sp := as.factor(sp)]
```
...
### Summarise patch switches

```{r}
# summarise patch switches by species and treatments
patch_switch_summary = patch_switches[treat != "Manipulated" | 
...
# save small data
fwrite(
  patch_switch_summary,
  file = "data/results/data_patch_switch_summary.csv"
)
```
...
ggplot(psdf)+
  ...
  labs(
    ...
    y = "Distance moved between residence patches (m) / hr",
    fill = NULL
  )
