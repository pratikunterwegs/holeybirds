...
### Function to read data by species

```{r}
read_species = function(path = "data/processed/data_id/", 
                        sp_name, index) {
  files = list.files("data/processed/data_id/", full.names = TRUE)
  file = stringi::stri_subset_regex(files, pattern = sp_name)[index]
...
```{r}
data_raw = read_species(sp_name = "Pycnonotus", index = 19)
...
data_proc = data_proc[speed_in < 15 & speed_out < 15, ]
...
# select columns
data_thin = data_proc[, c("TAG_ID", "sp", "treat", 
                          "UNIX", "x", "y")]
...
data_thin[, c("day", "hour") := list(
  lubridate::date(as.POSIXct(time, origin = "1970-01-01")),
  lubridate::hour(as.POSIXct(time, origin = "1970-01-01"))
)]
