...
# 
# # filter data
patch_switches = patch_switches[treat != "NonMoulting", ]
...
# rescale rrv across species and treatment
psdf[, c("rrv_sc") := list(
  scales::rescale(rrv_calc)
  )
  ,by = c("sp", "treat")
]
```
...
```{r}
fig_patch_switches = ggplot(psdf)+
  geom_jitter(
    aes(
