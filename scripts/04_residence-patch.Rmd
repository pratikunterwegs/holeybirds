...
Get residence patch spatials.

```{r}
# run summary function over each list element
patch_data = lapply(patch_data, rbindlist) |> rbindlist()

patch_spatials = atl_patch_summary(
...
st_write(
  patch_spatials,
  dsn = "data/spatial/data_patch_spatials.gpkg"
)
```
